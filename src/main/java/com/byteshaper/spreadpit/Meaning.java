...
public class Meaning {
  ...
  @SafeVarargs
  public static Set<Meaning> mergeMeanings(List<Meaning>... meaningLists) {
    final List<Meaning> allMeanings = new ArrayList<>();
    Arrays.stream(meaningLists).forEach(list -> allMeanings.addAll(list));
    List<Meaning> allProcessedMeanings = allMeanings.stream().sorted(
        (m0, m1) -> m0.words.stream().reduce(wordsReducer()).orElse("").compareTo(
            m1.words.stream().reduce(wordsReducer()).orElse(""))).collect(Collectors.toList());

    Set<Meaning> resultSet = new LinkedHashSet<>();

    for (Meaning meaning : allProcessedMeanings) {
